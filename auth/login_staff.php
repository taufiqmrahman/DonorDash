...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $u = $_POST['username'];
    $p = $_POST['password'];
    $stmt = $conn->prepare("SELECT Staff_ID, Role, Password FROM Hospital_Staff WHERE Username = ? AND Role IN ('Medical Officer', 'Data Clerk')");
    $stmt->bind_param("s", $u);
    $stmt->execute();
    $res = $stmt->get_result();
    if ($row = $res->fetch_assoc()) {
        if ($p === $row['Password'] || password_verify($p, $row['Password'])) {
            ...
            if ($row['Role'] === 'Medical Officer') {
                header("Location: ../dashboards/staff_dash.php");
            } else {
                header("Location: ../dashboards/clerk_dash.php");
            }
            exit();
        }
    }
    $err = "Invalid Staff credentials or unauthorized role.";
}
?>
<!DOCTYPE html>
<html lang="en" >
<head>
    <meta charset="UTF-8"><title>Staff Login - DonorDash</title>
...
<body class="app-page bg-slate-50 text-slate-100 flex items-center justify-center min-h-screen">
    <div class="bg-white border border-slate-200 p-8 rounded-2xl w-full max-w-md shadow-2xl">
        <h2 class="text-xl font-bold mb-1 text-slate-900">Staff Portal Login</h2>
        <p class="text-xs text-slate-600 mb-6">Medical Officers & Data Clerks access.</p>
        ...
        <form method="POST" class="space-y-4">
            ...
            <button type="submit" class="app-button w-full bg-red-600 hover:bg-red-700 text-slate-900 font-medium py-3 rounded-lg text-sm transition">Sign In</button>
        </form>
